...
def read_video_frames_opencv(path, clip_len=32, resize=(112, 112), stride=2):
    ...
    # sample frames: take clip_len frames uniformly spaced across the video
...
def frames_to_tensor(frames_np):
    # frames_np: [T, H, W, C] -> tensor [1, T, C, H, W]
    t = torch.from_numpy(frames_np).permute(0, 3, 1, 2).contiguous()  # [T, C, H, W]
    # Use the training normalization by default (matches src/data/wlasl_ds.py)
    if st.sidebar.checkbox('Use training normalization (mean=0.45,std=0.225)', value=True):
...
available_ckpts = sorted([p.name for p in ckpt_dir.glob('*.pt')])

ckpt_choice = sidebar.selectbox('Select checkpoint from `checkpoints/`', options=['-- none --'] + available_ckpts)
uploaded_ckpt = sidebar.file_uploader('Or upload a checkpoint (.pt)', type=['pt', 'pth'])
...
def _load_ckpt_and_store(ckpt_path):
    try:
        m = R2Plus1D18WithPermute(num_classes=num_classes, pretrained=pretrained_flag)
...
if st.button('Load selected checkpoint'):
    try:
        ...
        with st.spinner('Loading checkpoint...'):
            _load_ckpt_and_store(ckpt_path)
    except Exception as e:
        st.exception(e)
    ...
    if auto_ckpt is not None:
        try:
            with st.spinner(f'Auto-loading checkpoint {Path(auto_ckpt).name} ...'):
                _load_ckpt_and_store(auto_ckpt)
        except Exception:
...
if mode == 'Upload video':
    ...
    try:
        ...
        if manifest_df is None:
            # try to load the default manifest if present
            default_manifest = Path('data/wlasl_preprocessed/manifest_nslt2000_roi_top104_balanced_clean.csv')
            if default_manifest.exists():
                manifest_df = pd.read_csv(default_manifest)
                if 'label_new' in manifest_df.columns and 'gloss' in manifest_df.columns:
            ...
            if m is None:
                ...
                try:
                    with st.spinner('Reading and preprocessing video...'):
                        frames = read_video_frames_opencv(tmp_path, clip_len=32, resize=(112,112))
                        tensor = frames_to_tensor(frames)  # [1, T, C, H, W]
                    ...
                    # check whether this uploaded video's filename appears in the manifest paths
                    true_gloss = None
                    in_manifest = False
                    manifest_row_path = None
                    manifest_label = None
                    try:
                        # ensure we have a manifest dataframe to lookup; if not, try default manifest file
                        local_manifest = manifest_df
                        default_manifest = Path('data/wlasl_preprocessed/manifest_nslt2000_roi_top104_balanced_clean.csv')
                        if local_manifest is None and default_manifest.exists():
                            try:
                                local_manifest = pd.read_csv(default_manifest)
                            except Exception:
                                local_manifest = None
                        if local_manifest is not None and 'path' in local_manifest.columns:
                            # compare by basename first for robustness
                            vname = Path(tmp_path).name
                            matches = local_manifest[local_manifest['path'].apply(lambda x: Path(str(x)).name == vname)]
                            if matches.empty:
                                # try endswith (in case manifest stores directories)
                                matches = local_manifest[local_manifest['path'].apply(lambda x: str(x).endswith(vname))]
                            if not matches.empty:
                                in_manifest = True
                                r = matches.iloc[0]
                                manifest_row_path = str(r.get('path'))
                                manifest_label = int(r.get('label_new')) if 'label_new' in r.index else None
                                true_gloss = r.get('gloss') if 'gloss' in r.index else None
                    except Exception:
                        pass
            ...
            if m is None:
                ...
                try:
                    with st.spinner('Reading and preprocessing selected sample...'):
                        frames = read_video_frames_opencv(sample_path, clip_len=32, resize=(112,112))
                        tensor = frames_to_tensor(frames)  # [1, T, C, H, W]
                    with st.spinner('Running inference...'):
                        pred, conf, probs = predict_tensor(m, tensor)
                    ...
                    # manifest lookup for the sample
                    true_gloss = None
                    in_manifest = False
                    manifest_row_path = None
                    manifest_label = None
                    try:
                        if manifest_df is not None and 'path' in manifest_df.columns:
                            vname = sample_path.name
                            matches = manifest_df[manifest_df['path'].apply(lambda x: Path(str(x)).name == vname)]
                            if matches.empty:
                                matches = manifest_df[manifest_df['path'].apply(lambda x: str(x).endswith(vname))]
                            if not matches.empty:
                                in_manifest = True
                                r = matches.iloc[0]
                                manifest_row_path = str(r.get('path'))

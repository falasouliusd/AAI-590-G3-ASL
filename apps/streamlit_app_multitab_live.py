...
from src.utils.live_roi import LiveROICropper
...
def ensure_best_checkpoint_loaded(prefix, placeholder=None):
    if not BEST_CKPT_PATH.exists():
        if placeholder is not None:
            placeholder.error(f'Missing checkpoint: {BEST_CKPT_PATH}')
        return False
    keys = context_keys(prefix)
    current_ckpt = st.session_state.get(keys['ckpt'])
    model_present = st.session_state.get(keys['model'])
    if model_present is None or current_ckpt != str(BEST_CKPT_PATH):
        with st.spinner('Loading best checkpoint...'):
            return load_checkpoint_into_context(prefix, BEST_ARCH, BEST_CKPT_PATH, BEST_NUM_CLASSES, True, placeholder)
    return True
...
class LiveVideoProcessor(VideoTransformerBase):
    def __init__(self, context_prefix, norm_mode='training', top_k=3):
        self.context_prefix = context_prefix
        self.norm_mode = norm_mode
        self.top_k = top_k
        self.buffer = []
        self.last_infer_ts = 0.0
        st.session_state.setdefault('live_translation', None)
        self.roi_cropper = LiveROICropper()

    def _run_inference(self):
        keys = context_keys(self.context_prefix)
        model = st.session_state.get(keys['model'])
        if model is None or len(self.buffer) < LIVE_CLIP_LEN:
            return
        arr = np.stack(self.buffer[-LIVE_CLIP_LEN:], axis=0).astype(np.float32) / 255.0
        tensor = frames_to_tensor(arr, skip_normalize=False, norm_mode=self.norm_mode)
        try:
            pred, conf, probs = predict_tensor(model, tensor)
        except Exception:
            return
        probs_arr = np.array(probs)
        topk_idx = probs_arr.argsort()[::-1][:self.top_k]
        lines = []
        for idx in topk_idx:
            gloss = label_map.get(int(idx)) if label_map is not None else str(int(idx))
            lines.append({'label': int(idx), 'gloss': gloss, 'confidence': float(probs_arr[idx])})
        translation = {
            'label': lines[0]['label'],
            'gloss': lines[0]['gloss'],
    ...
    def transform(self, frame: av.VideoFrame):
        ...
        crop_bgr = frame_bgr
        if self.roi_cropper and self.roi_cropper.enabled():
            try:
                crop_bgr = self.roi_cropper.crop(frame_bgr)
            except Exception:
                crop_bgr = frame_bgr
        ...
        if len(self.buffer) > LIVE_CLIP_LEN:
            self.buffer = self.buffer[-LIVE_CLIP_LEN:]
        now = time.time()
        if len(self.buffer) == LIVE_CLIP_LEN and (now - self.last_infer_ts) > 1.0:
            self.last_infer_ts = now
            self._run_inference()
        display = crop_bgr
        try:
            display = cv2.resize(crop_bgr, (frame_bgr.shape[1], frame_bgr.shape[0]))
        except Exception:
            display = crop_bgr
        return display
...
quick_tab, advanced_tab, live_tab = st.tabs(["Quick Demo", "Advanced Mode", "Live Streaming"])
...
with quick_tab:
    ...
    status_box = st.empty()
    ensure_best_checkpoint_loaded('quick', status_box)
...
# ---------------------- Live Streaming Tab ----------------------
with live_tab:
    st.subheader('Live Streaming — Webcam Demo')
    st.write('Stream gestures directly from your webcam. The best VideoResNet checkpoint is auto-loaded.')
    live_status = st.empty()
    ensure_best_checkpoint_loaded('live', live_status)
    def clear_live_translation():
        st.session_state['live_translation'] = None
    st.button('Clear translation', on_click=clear_live_translation, key='live_clear_btn')

    cam_col, trans_col = st.columns([3, 2])
    with cam_col:
        ...
        webrtc_streamer(
            ...
            video_processor_factory=lambda: LiveVideoProcessor('live', norm_mode='training', top_k=3),
        )
    with trans_col:
        live_translation = st.session_state.get('live_translation')
        st.write('Translation')
        if live_translation:
            lines = live_translation.get('top_k', [])
            preview = f"{live_translation['gloss']} (label {live_translation['label']}) — {live_translation['confidence']:.2f}"
            st.success(preview)
            st.json({'top_k': lines})
        else:
            st.info('No translation yet — start the webcam to see predictions.')

    debug_expander('live', 'Live Streaming Debug Info')
